// ==UserScript==
// @name         MediaHunter 媒体猎手 (V6.0 稳定性能版+请求头参数)
// @namespace    http://tampermonkey.net/
// @version      6.0
// @description  Capture video and audio resources. High-performance UI rendering & memory management. Reverted stateful /g regex to local scope to guarantee async stability. Short media filter <=3s. Smart DOM physical size sync.
// @author       yudong2ao
// @match        *://*/*
...
(function () {
    ...
    const REGEX_4K = /(?:-|_|\/|@)(4k)(?:\/|_|-|\?|&|\.|$)/i;
    const REGEX_2K = /(?:-|_|\/|@)(2k)(?:\/|_|-|\?|&|\.|$)/i;
    ...
    const ICONS = {
        ...
        audio: "data:image/svg+xml;base64,",
    ...
    function guessResolutionFromUrl(url) {
        ...
        if (REGEX_4K.test(url)) return "4K";
        if (REGEX_2K.test(url)) return "2K";
        return "";
    }
    ...
    async function parseM3u8(url, visited = new Set(), inheritedMeta = { width: 0, height: 0 }) {
        ...
        try {
            ...
            // 安全：局部正则，无惧异步打断导致的 lastIndex 污染
    ...
    function addMedia(media) {
        ...
        if (existingIndex !== -1) {
            ...
            if ((!existing.duration && media.duration) || (!existing.resolution && media.resolution)) {
                if (!media.isPageUrl && (!media.duration || media.duration <= 6)) {
        ...
        if (!media.isPageUrl && (!media.duration || media.duration <= 6)) return;
    ...
    function extractUrlsFromTextAsync(text) {
        ...
        schedule(() => {
            ...
            while ((match = regexUrl.exec(text)) !== null) {
                if (!STATE.sniffedUrls.has(match[0])) checkNetworkResource(match[0], null);
            }
        }, { timeout: 2000 });
    }
    ...
    function syncDomRes(activeRes, activeWidth, activeHeight) {
        ...
        if (videos.length === 1) {
            if (videos[0].resolution !== activeRes) {
                videos[0].resolution = activeRes;
                updated = true;
            }
        } else if (videos.length > 1) {
            videos.forEach(m => {
                let isMatch = false;
                if (!m.resolution) isMatch = true;
                else if (m.resolution === `${activeHeight}P`) isMatch = true;
                else if (m.resolution === `${activeWidth}P`) isMatch = true;
                else if (m.resolution === `4K` && (activeHeight >= 2160 || activeWidth >= 2160)) isMatch = true;
                else if (m.resolution === `2K` && (activeHeight >= 1440 || activeWidth >= 1440)) isMatch = true;

                if (isMatch && m.resolution !== activeRes) {
    ...
    function scanDOM() {
        ...
        document.querySelectorAll('video, audio, source').forEach(el => {
            ...
            const createMediaItem = () => {
                ...
                if (!res) res = guessResolutionFromUrl(src);
                return {
    ...
    async function checkNetworkResource(url, contentType) {
        ...
        if (mediaItem.ext === 'm3u8') {
            ...
            if (metadata) { mediaItem.duration = metadata.duration; mediaItem.width = metadata.width; mediaItem.height = metadata.height; mediaItem.resolution = metadata.resolution; }
        } else if (!isKnown) {
            const meta = await probeMediaMetadata(url, mediaItem.type);
            if (meta) { mediaItem.duration = meta.duration; mediaItem.width = meta.width; mediaItem.height = meta.height; mediaItem.resolution = getResolution(meta.width, meta.height); }
        }

        if (!mediaItem.resolution) mediaItem.resolution = guessResolutionFromUrl(url);
