// ==UserScript==
// @name         MediaHunter 媒体猎手 (V8.1.6 极致性能版)
...
(function () {
    ...
    // 【性能优化 1 & 3】解耦转义逻辑，加入 chunking。按需对字符串做转义替换，节省内存和主线程耗时
    function extractUrlsFromTextAsync(text, sourceSessionId = PAGE_SESSION_ID, needsUnescape = true) {
        ...
        schedule(() => {
            ...
            // 如果不是 script/json（例如纯 Body），直接不执行高耗时的全局 replace
    ...
    // 【性能优化 3】利用任务切片，细化文本提取的颗粒度，消除由于单次执行导致的页面卡顿现象
            ...
            setTimeout(() => {
                ...
                if (document.body && document.body.innerHTML) {
                    ...
                    if (bodyHtml.length < 3 * 1024 * 1024) {
                        // HTML DOM 节点里的 URL 通常不需要清理转义 \/ (false)，这省去了极为昂贵的 3MB 字符串 replace 开销
