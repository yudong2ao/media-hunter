// ==UserScript==
// @name         MediaHunter 媒体猎手 (V8.2.10 按钮展示逻辑优化)
// @namespace    http://tampermonkey.net/
// @version      8.2.10
// @description  Strict rules for ghost resources. Intercept native payload. Extreme performance optimization with chunking and LRU cache. Local API Architecture integration. Auto-close on download success. Bulletproof MPD XML namespace and ISO-8601 duration parser. [V8.2.10] 动态悬浮按钮，无有效资源时自动隐藏，加入防闪烁延迟机制。
...
(function () {
    ...
    if (window.trustedTypes && window.trustedTypes.createPolicy) {
        try {
            ttPolicy = window.trustedTypes.createPolicy("mh-trusted-policy", {
    ...
    function safeHTML(htmlStr) {
        return ttPolicy ? ttPolicy.createHTML(htmlStr) : htmlStr;
    }

    const INTERCEPTED_TEXTS = new Map();
    // 纯净内容透视方案：用于标记内容实为 M3U8 但被伪装的原始 URL
    const FORCE_M3U8_URLS = new Set();
    ...
    const YTDLP_SUPPORTED_SITES = [
        'youtube.com', 'tiktok.com', 'instagram.com', 'facebook.com', 'x.com', 'twitter.com', 'reddit.com', 'vimeo.com', 'dailymotion.com', 'rumble.com', 'bitchute.com', 'odysee.com', 'bilibili.com', 'acfun.cn', 'douyin.com', 'xiaohongshu.com', 'kuaishou.com', 'weibo.com', 'cctv.com', 'ixigua.com', 'video.baidu.com', 'tv.sohu.com', 'v.qq.com', 'iqiyi.com', 'youku.com', 'douyu.com', 'huya.com', 'pearvideo.com', 'nicovideo.jp', 'abema.tv', 'tver.jp', 'tv.naver.com', 'tv.kakao.com', 'openrec.tv', 'soundcloud.com', 'bandcamp.com', 'mixcloud.com', 'music.163.com', 'ted.com', 'udemy.com', 'coursera.org', 'khanacademy.org', 'cnn.com', 'bbc.com', 'foxnews.com', 'bloomberg.com', 'aljazeera.com', 'reuters.com', 'nhk.or.jp', 'pornhub.com', 'xvideos.com', 'xhamster.com', 'spankbang.com', 'youporn.com', 'redtube.com', 'chaturbate.com', 'fc2.com'
    ];
    ...
    const STYLES = `
        ...
        .mh-btn { outline: none !important; box-shadow: none !important; width: 42px !important; height: 20px !important; border: none !important; border-radius: 4px !important; font-size: 11px !important; cursor: pointer !important; display: flex !important; justify-content: center !important; align-items: center !important; padding: 0 !important; margin: 0 !important; font-family: inherit !important; font-weight: 500 !important; transition: all 0.2s ease !important; line-height: 1 !important; }
        .mh-btn:focus, .mh-btn:active { outline: none !important; box-shadow: none !important; }
        .mh-btn-copy { background: #e9ecef !important; color: #495057 !important; width: 36px !important;}
    ...
    const EXT_MAP = new Map([
        ...
        ["mkv", true], ["avi", true], ["mov", true], ["flac", true], ["mka", true], ["m4s", true], ["mpd", true]
    ]);
    ...
    function detectFormat(url, contentType) {
        // [核心修复点] 强制内容透视映射：如果在标记池中，无视一切强制赋予 M3U8 属性
        if (FORCE_M3U8_URLS.has(url)) {
    ...
    function getMediaType(format, url) {
        ...
        if (format.ext === 'mpd') return 'video';
        return 'video';
    }
    ...
    async function parseMpd(url, visited = new Set(), inheritedMeta = { width: 0, height: 0 }) {
        if (visited.has(url)) return null;
        visited.add(url);
        try {
            const text = await requestText(url);
            ...
            let duration = 0;
            const mpdNode = xmlDoc.documentElement;
            if (mpdNode && (mpdNode.localName === "MPD" || mpdNode.tagName.includes("MPD"))) {
                const mediaPresentationDuration = mpdNode.getAttribute("mediaPresentationDuration");
                if (mediaPresentationDuration) {
                    duration = parseIsoDuration(mediaPresentationDuration);
                }
            }

            if (duration === 0) {
                const periods = xmlDoc.getElementsByTagNameNS ? xmlDoc.getElementsByTagNameNS("*", "Period") : xmlDoc.getElementsByTagName("Period");
                for (let i = 0; i < periods.length; i++) {
                    const d = periods[i].getAttribute("duration");
                    if (d) duration += parseIsoDuration(d);
                }
            }

            let maxWidth = inheritedMeta.width || 0;
            let maxHeight = inheritedMeta.height || 0;

            const representations = xmlDoc.getElementsByTagNameNS ? xmlDoc.getElementsByTagNameNS("*", "Representation") : xmlDoc.getElementsByTagName("Representation");
            for (let i = 0; i < representations.length; i++) {
                const rep = representations[i];
                const w = parseInt(rep.getAttribute("width") || 0);
                const h = parseInt(rep.getAttribute("height") || 0);
            ...
            return {
                ...
                width: maxWidth,
                height: maxHeight,
                resolution: getResolution(maxWidth, maxHeight)
            };
        } catch (e) { return inheritedMeta; }
    }
    ...
    function addMedia(media) {
        ...
        if (existingIndex !== -1) {
            ...
            if (needsUpdate) {
                ...
                STATE.mediaList[existingIndex] = { ...existing, ...media, id: existing.id, _copyUntil: existing._copyUntil, _dlUntil: existing._dlUntil, _copyText: existing._copyText, _dlErrUntil: existing._dlErrUntil, _dlErrText: existing._dlErrText };
    ...
    function extractUrlsFromTextAsync(text, sourceSessionId = PAGE_SESSION_ID, needsUnescape = true) {
        ...
        schedule(() => {
            ...
            const regexUrl = /https?:\/\/[^"'\s<>\{\}\[\]\\]+\.(?:m3u8|mpd|mp4|flv|m4a|m4s)(?:\?[^"'\s<>\{\}\[\]\\]*)?/gi;
            let match;
    ...
    function setupNetworkSniffing() {
        ...
        window.fetch = async function (input, init) {
            ...
            try {
                ...
                const isManifest = url.includes('.m3u8') || url.includes('.mpd') || (contentType && (contentType.includes('mpegurl') || contentType.includes('application/x-mpegurl') || contentType.includes('dash+xml')));
                const isTextOrJson = contentType && (contentType.includes('json') || contentType.includes('text'));

                if (isManifest || isTextOrJson) {
                    clone.text().then(text => {
                        // [核心修复点] 如果是伪装 M3U8，打上内部标记
                        if (text && typeof text === 'string' && text.trim().startsWith('#EXTM3U')) {
                            FORCE_M3U8_URLS.add(url);
                        }

                        if (url.includes('.m3u8') || FORCE_M3U8_URLS.has(url)) {
                        ...
                        if (isManifest || isTextOrJson) {
                            if (this.responseType === '' || this.responseType === 'text') {
                                if (this.responseText) {
                                    // [核心修复点] 如果是伪装 M3U8，打上内部标记
                                    if (typeof this.responseText === 'string' && this.responseText.trim().startsWith('#EXTM3U')) {
                                        FORCE_M3U8_URLS.add(url);
                                    }
    ...
    function scheduleDurationRetry(mediaItem, sessionId) {
        [2000, 5000, 9000].forEach(delay => {
            setTimeout(async () => {
                ...
                } else if (mediaItem.ext === 'mpd') {
                    const metadata = await parseMpd(mediaItem.url);
                    if (metadata) {
                        if (metadata.duration > 0) newDuration = metadata.duration;
                        if (metadata.resolution && (!newRes || resGuessed)) {
                            newRes = metadata.resolution;
    ...
    async function checkNetworkResource(url, contentType, sourceSessionId = PAGE_SESSION_ID) {
        ...
        if (mediaItem.ext === 'm3u8') {
            ...
            const metadata = await parseMpd(url);
            if (PAGE_SESSION_ID !== reqSessionId) return;
            if (metadata) {
                mediaItem.duration = metadata.duration;
                mediaItem.width = metadata.width;
    ...
    function initUI() {
        ...
        btn.id = 'mh-float-btn';
        btn.innerHTML = safeHTML(`<div class="mh-logo-icon">${ICONS.logo}</div><span>媒体猎手</span>`);
        btn.onclick = togglePopup;
        root.appendChild(btn);

        const popup = document.createElement('div');
        popup.id = 'mh-popup';
        popup.innerHTML = safeHTML(`
    ...
    let buttonVisibilityTimer = null;
    ...
    function updateButtonVisibility() {
        ...
        if (buttonVisibilityTimer) clearTimeout(buttonVisibilityTimer);

        buttonVisibilityTimer = setTimeout(() => {
            const renderable = getRenderableMedia();
            const shouldShow = renderable.length > 0 || isKnownYtDlpSite();
    ...
    function renderCards() {
        ...
        const displayList = getRenderableMedia();
        ...
        displayList.forEach(media => {
            if (media.isPageUrl) {
                pageList.push(media);
            } else if (media.ext === 'm3u8' || media.ext === 'mpd') {
                m3u8List.push(media);
            } else {
        ...
        sortedList.forEach(media => {
            ...
            if (media._dlUntil && now < media._dlUntil) {
                currentDlText = "下载中";
                dlBtnStyle = "background: #2b8a3e !important;";
            } else if (media._dlErrUntil && now < media._dlErrUntil) {
                currentDlText = media._dlErrText || "未连接";
                dlBtnStyle = "background: #e03131 !important;";
            }
            ...
            card.innerHTML = safeHTML(`
                ...
                <div class="mh-card-bottom">
                    ...
                    <div class="mh-actions">
                        ...
                        <button class="mh-btn mh-btn-ytdlp" id="mh-dl-${media.id}" style="${dlBtnStyle}">${currentDlText}</button>
                    </div>
                </div>
            `);
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                        if (response.status === 200) {
                            ...
                            media._dlErrUntil = Date.now() + 2000;
                            media._dlErrText = "异常";
                            scheduleRender();
