...
// @version      2.9
// @description  Capture video and audio resources. Enhanced parsing. Strictly conform to yt-dlp command line arguments. UI strictly matches the new design mockup (Neutral Gray Secondary Button).
...
(function () {
    ...
    const STYLES = `
        ...
        /* 【重点更新】方案 C：中性灰度复制按钮 */
        .mh-btn-copy {
            background: #e9ecef; /* 浅灰背景 */
            color: #495057;      /* 深灰文字 */
        }
        .mh-btn-copy:hover {
            background: #dee2e6; /* hover 时背景稍微加深 */
        }

        /* 下载按钮保持不变，稳居主视觉焦点 */
        .mh-btn-ytdlp {
            background: rgba(45, 33, 183, 0.8);
            color: white;
        }
        .mh-btn-ytdlp:hover {
    ...
    function renderCards() {
        ...
        [...STATE.mediaList].reverse().forEach(media => {
            ...
            let durationHtml = '';
            if (durationStr === "直播流") {
                durationHtml = `<span class="mh-tag-dur mh-tag-live">直播</span>`;
            } else if (durationStr) {
                durationHtml = `<span class="mh-tag-dur">${durationStr}</span>`;
            } else {
                 durationHtml = `<span class="mh-tag-dur">--:--</span>`;
            }

            let resText = media.resolution ? media.resolution : '--';
            let resHtml = `<span class="mh-tag-txt">${resText}</span>`;
            ...
            card.innerHTML = `
                ...
                <div class="mh-card-bottom">
                    <div class="mh-info-group">
                        ${durationHtml}
                        ${resHtml}
                        ${extHtml}
                    </div>
                    <div class="mh-actions">
                        <button class="mh-btn mh-btn-copy">复制</button>
            ...
            card.querySelector('.mh-btn-ytdlp').onclick = (e) => {
                ...
                const cmdArgs = `"${targetUrl}" --referer "${currentUrl}" -o "D:\\00暂存\\02媒体猎手\\${safeTitle}_%(id)s.%(ext)s"`;
